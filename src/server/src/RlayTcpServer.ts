  ...
  private handleSocketConnection(socket: Socket) {
    ...
    socket.once("data", (chunk) => {
      ...
      if (chunk.toString("utf8", 0, TcpCommand.Connect.length) === TcpCommand.Connect) {
        this.handleClientSocketConnectionAsync(new AsyncSocket(socket, this.config.password), chunk.toString()).then()
      } else {
        this.handleNewInboundConnectionAsync(new AsyncSocket(socket, this.config.password), chunk, socketId).then()
      }
    })
  }
